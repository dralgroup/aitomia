...
from .ir_static import ir_static_builder 
...
class IRPlannerState(AitomiaState):
    ir_task_sequence: List[str] = None
    task_type: str = None 
    
def prompt_ir_planner():
    prompt = "" 
    prompt += "You need to design a workflow according to the given messages. For IR spectrum calculation, you need to first optimize the molecule, then calculate its spectrum. The workflow is a list of tasks. Below shows all the available tasks: \n"
    ...
    for agent_name, agent_card in agent_cards.items():
        if agent_name in ["geomopt_agent","ir_static_agent"]:
...
PROMPT_IR_PLANNER = prompt_ir_planner()
...
def decide_task_type(
    ...
    Args:
        ...
        ir_static: Whether to calculate static IR spectrum of a molecule.
    """
    ...
    if ir_static: return "ir_static_node"
...
# Graph
#-------------------------------------------------
def ir_planner_node(state:IRPlannerState):
    logger.info("Start making a plan of the IR spectrum calculation")
    message = "Start making a plan of the IR spectrum calculation"
    
    ir_planner_prompt = SystemMessage(content=PROMPT_IR_PLANNER)
    ...
    # Now current_task_messages[-2] is the ir task message list, current_task_messages[-1] is the sub-task message list
    response = llm.invoke(current_task_messages[-2] + [ir_planner_prompt])
...
def workdir_manager_node(state:IRPlannerState):
    ...
    task_sequence = state.ir_task_sequence
    ...
    # Put the last message (summary) in the sub-task message list into the ir task message list
    ...
    return {
        "ir_task_sequence":task_sequence,
        "current_task_messages":task_messages,
        "result_files": state.result_files,
        "messages_to_user": [AIMessage(content=message)],
    }
    
def task_type_node(state:IRPlannerState) -> Command[Literal["geomopt_node","ir_static_node"]]:
    logger.info("Start choosing task type in IR calculation")
...
def ir_analysis_node(state:IRPlannerState):
...
def conditional_edge(state:IRPlannerState):
    ntasks = len(state.ir_task_sequence)
    logger.info(f"Number of remaining tasks for IR: {ntasks}")
    if ntasks == 0:
        return "ir_analysis_node"
    else:
        return "workdir_manager_node"
    
ir_builder = StateGraph(IRPlannerState)

geomopt_graph = geomopt_builder.compile()
ir_static_graph = ir_static_builder.compile()

ir_builder.add_node("get_folder_name_node",get_folder_name_node)
ir_builder.add_node("ir_planner_node",ir_planner_node)
ir_builder.add_node("workdir_manager_node",workdir_manager_node)
ir_builder.add_node("task_type_node",task_type_node)
ir_builder.add_node("geomopt_node",geomopt_graph)
ir_builder.add_node("ir_static_node",ir_static_graph)
ir_builder.add_node("ir_analysis_node",ir_analysis_node)

ir_builder.add_edge(START,"get_folder_name_node")
ir_builder.add_edge("get_folder_name_node","ir_planner_node")
ir_builder.add_edge("ir_planner_node","workdir_manager_node")
ir_builder.add_edge("workdir_manager_node","task_type_node")
...
ir_builder.add_edge("ir_analysis_node",END)

ir_graph = ir_builder.compile() 
from .agent_template import BaseAgent
ir_agent = BaseAgent(
    name='ir_agent',
    description='Perform infrared (IR) intensity calculation of a molecular structure.',
    graph=ir_builder,
)